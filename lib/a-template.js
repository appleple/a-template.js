...
(0, _zeptoBrowserify.$)(document).on("input change click", "[data-bind]", function (e) {
    var data = (0, _zeptoBrowserify.$)(undefined).data("bind");
    var val = (0, _zeptoBrowserify.$)(undefined).val();
    var attr = (0, _zeptoBrowserify.$)(undefined).attr("href");
    if (attr) {
        val = attr.replace("#", "");
    }
    var id = (0, _zeptoBrowserify.$)(undefined).parents("[data-id]").data("id");
    if (id) {
        var obj = getObjectById(id);
        if ((0, _zeptoBrowserify.$)(e.target).attr("type") == "radio") {
            if ((0, _zeptoBrowserify.$)(undefined).is(":checked")) {
                obj.updateDataByString(data, val);
            } else {
                obj.updateDataByString(data, '');
            }
        } else if ((0, _zeptoBrowserify.$)(e.target).attr("type") == "checkbox") {
            var name = (0, _zeptoBrowserify.$)(undefined).attr("name");
            var arr = [];
            (0, _zeptoBrowserify.$)(":checkbox[name=" + name + "]").each(function () {
                if ((0, _zeptoBrowserify.$)(undefined).is(":checked")) {
                    arr.push((0, _zeptoBrowserify.$)(undefined).val());
                }
            });
...
(0, _zeptoBrowserify.$)(document).on("input click change keydown", "[data-action]", function (e) {
    if (e.type == "click" && (0, _zeptoBrowserify.$)(e.target).is("select")) {
        return;
    }
    if (e.type == "input" && (0, _zeptoBrowserify.$)(e.target).attr("type") == "button") {
        return;
    }
    var string = (0, _zeptoBrowserify.$)(undefined).data("action");
    ...
    _createClass(aTemplate, [{
        ...
        value: function loadHtml() {
            ...
            templates.forEach(function (template) {
                var d = new _zeptoBrowserify.$.Deferred();
                promises.push(d);
                var src = (0, _zeptoBrowserify.$)("#" + template).attr("src");
                _zeptoBrowserify.$.ajax({
                    url: src,
                    type: 'GET',
                    dataType: 'text'
                }).success(function (data) {
                    (0, _zeptoBrowserify.$)("#" + template).html(data);
                    d.resolve();
                });
            });
            return _zeptoBrowserify.$.when.apply(_zeptoBrowserify.$, promises);
        }
    }, {
        key: "getData",
        ...
        value: function resolveInclude(html) {
            ...
            html = html.replace(include, function (m, key) {
                return (0, _zeptoBrowserify.$)("#" + key).html();
            });
            return html;
        }
    }, {
        ...
        value: function getHtml(selector) {
            var $template = (0, _zeptoBrowserify.$)(selector);
        ...
        value: function update(txt, part) {
            ...
            for (var i = 0, n = templates.length; i < n; i++) {
                ...
                var $target = (0, _zeptoBrowserify.$)("[data-id='" + tem + "']");
                if (!part || part == tem) {
                    if ($target.length == 0) {
                        var $newitem = (0, _zeptoBrowserify.$)("<div data-id='" + tem + "'></div>");
                        $newitem[renderWay](html);
                        (0, _zeptoBrowserify.$)(selector).after($newitem);
                    } else {
        ...
        value: function updateBindingData(part) {
            ...
            for (var i = 0, n = templates.length; i < n; i++) {
                ...
                if (!part || part == temp) {
                    var $template = (0, _zeptoBrowserify.$)("[data-id='" + temp + "']");
                    $template.find("[data-bind]").each(function () {
                        var data = that.getDataByString((0, _zeptoBrowserify.$)(this).data("bind"));
                        if ((0, _zeptoBrowserify.$)(this).attr("type") == "checkbox" || (0, _zeptoBrowserify.$)(this).attr("type") == "radio") {
                            if (data == (0, _zeptoBrowserify.$)(this).val()) {
                                (0, _zeptoBrowserify.$)(this).prop("checked", true);
                            }
                        } else {
                            (0, _zeptoBrowserify.$)(this).val(data);
                        }
                    });
                    if (part) {
                        break;
        ...
        value: function copyToClipBoard() {
            var copyArea = (0, _zeptoBrowserify.$)("<textarea/>");
            (0, _zeptoBrowserify.$)("body").append(copyArea);
...
if (typeof window !== "undefined") {
    !window.aTemplate && (window.aTemplate = aTemplate);
}
